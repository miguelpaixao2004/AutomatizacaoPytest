...
def test_filtro_pizza_retorna_apenas_pizzas(client):
    ...
    pratos = response.json()
    for prato in pratos:
        assert prato["categoria"].lower() == "pizza" # Robusto: valida a lógica do filtro

import pytest

# 1. Categorias inválidas retornam 422
@pytest.mark.parametrize("categoria_ruim", [
    "japonesa", 
    "churrasco", 
    "esoterica", 
    "fast-food"
...
# 2. IDs inexistentes retornam 404
@pytest.mark.smoke
@pytest.mark.parametrize("id_inexistente", [9999, 8888]) # O parametrize PRECISA estar aqui
def test_ids_inexistentes_retornam_404(client, id_inexistente):
    response = client.get(f"/pratos/{id_inexistente}")
    assert response.status_code == 404
