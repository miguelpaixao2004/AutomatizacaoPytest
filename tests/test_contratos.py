...
def test_contrato_get_prato_id(client):
    ...
    assert campos_obrigatorios.issubset(dados.keys()), "Campos obrigatórios ausentes no GET"
    
    assert isinstance(dados["id"], int)
    assert isinstance(dados["nome"], str)
    assert isinstance(dados["preco"], (float, int)) # Aceita ambos para evitar erro com números inteiros
